module.exports = (_field = 'query', addQuery = []) => {
  return (req, res, next) => {
    ...
    req[_field].pcnt = req[_field].pcnt || 10;
    ...
    let goPath = res.locals.currents[0] ? '/' + res.locals.currents[0] : '';
    goPath += res.locals.currents[1] ? '/' + res.locals.currents[1] : '';
    let goQuery = '';
    ...
    res.locals.goList = `${goPath}?${goQuery}&page=${req[_field].page}&pcnt=${req[_field].pcnt}`;
    res.locals.goPath = goPath;
    res.locals.goQuery = `${goQuery}&page=${req[_field].page}&pcnt=${req[_field].pcnt}`;
    res.locals.goView = `${goPath}/${req.params.id}?${goQuery}&page=${req[_field].page}&pcnt=${req[_field].pcnt}`;
    res.locals.goUpdate = `${goPath}/${req.params.id}`;

    res.locals.goLists = [
      { key: 'page', value: req[_field].page },
      { key: 'pcnt', value: req[_field].pcnt },
      { key: 'field', value: req[_field].field },
      { key: 'search', value: req[_field].search },
      { key: 'sort', value: req[_field].sort },
    ];
    if (res.locals.currents[0] === 'board' || res.locals.currents[1] === 'board' || res.locals.currents[1] === 'comment') {
